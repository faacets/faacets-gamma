...
final class ShapeImpImpAction[S <: Scenario with Singleton](val shape: Shape[S]) extends PermutationAction[Relabeling.Aux[S]] {
  import shape._

  def isFaithful: Boolean = true

  def size: Int = imprimitiveImprimitive.size

  def movedPointsUpperBound(r: Relabeling.Aux[S]): NNOption = NNSome(size - 1)

  def findMovedPoint(g: Relabeling.Aux[S]): NNOption = smallestMovedPoint(g) // TODO: optimize

  def actl(r: Relabeling.Aux[S], k: Int): Int = actr(k, r.inverse)
  ...
  def actrGen(k: Int, r: Relabeling.Aux[S]): Int = {
    val block = imprimitiveImprimitive.blockIndices(k).toInt
  ...
  def actr(k: Int, r: Relabeling.Aux[S]): Int =
...
final class ShapePriImpAction[S <: Scenario with Singleton](val shape: Shape[S]) extends PermutationAction[Relabeling.Aux[S]] {
  import shape._

  def isFaithful: Boolean = false

  def findMovedPoint(g: Relabeling.Aux[S]): NNOption = smallestMovedPoint(g)
...
final class ShapePriPriAction[S <: Scenario with Singleton](val shape: Shape[S]) extends PermutationAction[Relabeling.Aux[S]] {
  import shape._
...
final class Shape[S <: Scenario with Singleton:Witness.Aux] protected[core] {

  def scenario: S = valueOf[S]

  def parties: Seq[Party] = scenario.parties

  override def hashCode = parties.hashCode

  override def equals(any: Any) = any match {
    case that: Shape[_] => this.parties == that.parties
    case _ => false
  }
  ...
  lazy val impImpAction = new ShapeImpImpAction[S](this)

  lazy val priImpAction = new ShapePriImpAction[S](this)

  lazy val priPriAction = new ShapePriPriAction[S](this)
